...
frames guide injected into your system prompt: overview thumbnails in
`public/frames/<jobId>/overview/`, on-demand extraction via `node scripts/grab-frames.mjs`, and
mandatory composite verification with `<DebugFrame>` (from `src/components`; it auto-hides in the
final render). All extracted frames are in CANVAS coordinates. Black or empty screens are free
canvas; real content must be anchored to.
