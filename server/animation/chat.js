...
export function buildSystemAppend(job, styleSkill, framesSkill = "") {
  ...
  return [
    ...
    ...(job.seeFrames ? [
      "- FRAME-AWARE: this overlay draws ON the user's footage. Anchor drawings to what is actually on screen and only while it is there; the frames guide below is authoritative for the workflow (frames-map.json, the overview thumbnails, scripts/grab-frames.mjs, DebugFrame verification).",
    ] : []),
    raw
