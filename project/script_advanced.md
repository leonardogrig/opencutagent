# Script (advanced): Opus 5 vs Fable 5.1, which is cheaper for agentic coding

This is `script.md` with every `[OPEN: url]` link replaced by the actual screenshot of that page,
captured on 2026-09-01. The animation agent cannot browse, so it reads these images instead.

**How to read the markers**

- `[IMAGE: refs/<file>]` means: open that image, look at the region named, and build the animation
  around what is actually on screen there (highlight that row, that cell, that bar, that quote).
- Every image also carries its source URL, so the context of what the viewer is looking at is clear.
- `[SHOW TABLE n]` marks where the edit cuts to a table (unchanged from the original script).
- `(ANIMATION: ...)` is the core idea for the visual under that block (unchanged).

**Where the images live**

- Agent-visible (use these relative paths): `refs/<file>`
- On disk (project): `/Users/leonardomac/Documents/Youtube/NEW FOCUS/09/OpenCutAgent Animations/anim-mtjfcaxx/refs/`
- On disk (animation workspace): `~/.opencutagent/animation-kit/src/jobs/anim-mtjfcaxx/refs/`

## Image index

| File | What it shows | Source |
|---|---|---|
| `refs/01-anthropic-benchmark-table.jpg` | Anthropic's full benchmark comparison table (Fable 5.1 / Fable 5 / Opus 5 / GPT-5.6 Sol) | https://www.anthropic.com/claude-fable-and-mythos-5-1 |
| `refs/03a-pricing-fable-5-1.jpg` | Fable 5.1 pricing card: input $10, output $50, 5m cache write $12.50, 1h cache write $20, cache read $0.25 per MTok | https://platform.claude.com/docs/en/models/fable-5-1/overview |
| `refs/03b-pricing-opus-5.jpg` | Opus 5 pricing card: input $5, output $25, 5m cache write $6.25, 1h cache write $10, cache read $0.50 per MTok | https://platform.claude.com/docs/en/models/opus-5/overview |
| `refs/04a-ccusage-daily-token-breakdown.jpg` | A real daily ccusage-style table with the four token columns: 340 input, 84,592 output, 303,910 cache create, 9,087,615 cache read (cache reads about 96% of tokens) | https://docs.rs/crate/cc-usage/latest (substitute, see Link notes) |
| `refs/04b-ccwhy-fixed-overhead-97pct.jpg` | ccwhy example output with the "Fixed Overhead" line: 97.2%, 3.2B cache reads | https://docs.rs/crate/ccwhy/latest |
| `refs/05-anthropic-indexed-cost-chart.jpg` | Anthropic's indexed cost chart: typical workload 100 to 75, highly agentic 100 to 55, split into cache reads vs all other tokens | https://www.anthropic.com/claude-fable-and-mythos-5-1 |
| `refs/06-devto-mcp-token-cost-table.jpg` | MCP token measurement table: github (official) 26 tools, about 3,100 tokens/turn, about 62,000 per 20-turn session | https://dev.to/wartzarbee/i-added-mcp-servers-to-claude-code-heres-what-they-cost-in-tokens-2can |
| `refs/08a-anthropic-quote-every.jpg` | Partner quote card, Every (Dan Shipper): "used half as many tokens" | https://www.anthropic.com/claude-fable-and-mythos-5-1 |
| `refs/08b-anthropic-quote-browserbase.jpg` | Partner quote card, Browserbase (Miguel Gonzalez): "while using fewer tokens than either" | https://www.anthropic.com/claude-fable-and-mythos-5-1 |
| `refs/08c-anthropic-quote-rogo.jpg` | Partner quote card, Rogo (Alex Wang): "matches Fable 5 on accuracy while using 20% fewer tokens" | https://www.anthropic.com/claude-fable-and-mythos-5-1 |
| `refs/09-anthropic-tbs-accuracy-vs-cost.jpg` | Terminal-Bench-Science accuracy vs cost chart, both curves labelled low / med / high / xhigh / max | https://www.anthropic.com/claude-fable-and-mythos-5-1 |
| `refs/10a-aa-linkedin-post-text.jpg` | The Artificial Analysis post text: 66 at max, $3.76 per task, 20% more than Fable 5, about 1.7x output tokens, cache cut saves about $1.40, xhigh 65 at $2.72, Opus 5 (max, 63) at $2.34 | https://www.linkedin.com/posts/artificial-analysis_claude-fable-51-tops-the-artificial-analysis-activity-7500649293469335552-6g83 |
| `refs/10b-aa-intelligence-vs-cost-per-task.png` | AA scatter, Anthropic vs OpenAI only: every Fable 5.1 and Opus 5 effort level plotted on score vs cost per task | same post |
| `refs/10c-aa-intelligence-vs-cost-all-models.png` | The same scatter with every lab on it, for the wider frame | same post |
| `refs/11-aa-intelligence-vs-output-tokens.png` | AA scatter, score vs OUTPUT TOKENS per task: this is the 1.7x evidence | same post |
...
[IMAGE: refs/01-anthropic-benchmark-table.jpg]
Anthropic's benchmark table on the Fable 5.1 launch page. Four model columns (Fable 5.1, Fable 5, Opus 5, GPT-5.6 Sol),
the Fable 5.1 column tinted green. Rows to point at: "Agentic scientific research / Terminal-Bench-Science 0.1"
(52.6% vs 29.0%, the big gap) and the two agentic coding rows, "Terminal-Bench 4.0" (55.8% vs 52.3%) and
"CursorBench 3.2.0" (73.4% vs 70.0%), which are the small gaps.
...
[IMAGE: refs/03a-pricing-fable-5-1.jpg]
The Fable 5.1 model page in the Claude Platform docs, Pricing card (dark UI, left column). The rows, in order:
Input $10 / MTok, Output $50 / MTok, 5m cache write $12.50 / MTok, 1h cache write $20 / MTok, Cache read $0.25 / MTok.
Source: https://platform.claude.com/docs/en/models/fable-5-1/overview

[IMAGE: refs/03b-pricing-opus-5.jpg]
The same Pricing card on the Opus 5 model page, for the side by side: Input $5, Output $25, 5m cache write $6.25,
1h cache write $10, Cache read $0.50 / MTok. The one row where Opus is the EXPENSIVE one is "Cache read".
...
[IMAGE: refs/04a-ccusage-daily-token-breakdown.jpg]
A published daily usage table with the four token columns side by side: Date, Models, Input, Output,
Cache Create, Cache Read, Total Tokens, Cost. The row reads 340 input, 84,592 output, 303,910 cache create,
9,087,615 cache read out of 9,476,457 total, $8.56 for the day. The point to draw: the Cache Read column
swallows the row (about 96% of the tokens).
Source: https://docs.rs/crate/cc-usage/latest
NOTE: the link in the original script (docs.rs/crate/ccusage-rs/latest) is a dead crate, see "Link notes" at the end.
...
[IMAGE: refs/04b-ccwhy-fixed-overhead-97pct.jpg]
The ccwhy example output block. The line to point at is under "Fixed Overhead (normal, not actionable)":
"97.2%  3.2B  Cache reads (context re-read every turn - normal, cheap at $1.5/M)". Above it, for contrast,
"Controllable Token Sinks ... Total controllable: 93.7M (2.8% of all tokens)".
Source: https://docs.rs/crate/ccwhy/latest
...
[IMAGE: refs/05-anthropic-indexed-cost-chart.jpg]
The "Indexed cost of Fable usage" chart from the Cost and availability section. Two panels: "Typical workload"
(Fable 5 = 100, Fable 5.1 = 75, labelled "~25% less") and "Highly agentic workload" (100 to 55, "~45% less").
Each bar is split into two parts by the legend at the top: hatched = "Cache reads", solid = "All other tokens".
The hatched part is the piece that shrinks.
Source: https://www.anthropic.com/claude-fable-and-mythos-5-1
...
[IMAGE: refs/06-devto-mcp-token-cost-table.jpg]
The MCP measurement table from the dev.to post, under "Here's what I found across 20-turn sessions".
Columns: MCP server, Tools registered, Tokens/turn (tool defs), 20-turn session overhead.
Rows: No MCP 0/0/0, Custom minimal server 3 tools / ~180 / ~3,600, filesystem (official) 7 / ~640 / ~12,800,
github (official) 26 / ~3,100 / ~62,000. The github row is the one the narration cites.
...
[IMAGE: refs/10a-aa-linkedin-post-text.jpg]
The Artificial Analysis post itself (dark LinkedIn UI), headline "Claude Fable 5.1 tops the Artificial Analysis
Intelligence Index but costs 20% more per task than Fable 5 despite a 75% cache read price cut". The key
takeaway bullet carries every number in table 7: Fable 5.1 (max) $3.76, about 1.7x output tokens, cache cut
saves about $1.40 per task, xhigh 65 at $2.72, Opus 5 (max, 63) at $2.34.
Source: https://www.linkedin.com/posts/artificial-analysis_claude-fable-51-tops-the-artificial-analysis-activity-7500649293469335552-6g83

[IMAGE: refs/10b-aa-intelligence-vs-cost-per-task.png]
The chart to build the scatter animation from: "Intelligence Index vs. Cost per Intelligence Index Task",
filtered to Anthropic and OpenAI. Every Fable 5.1 and Opus 5 effort level is a labelled dot, cost on a log
x-axis. Note the shape the narration describes: the Fable 5.1 dots sit up and to the RIGHT of Opus 5 (max).
Source: same post

[IMAGE: refs/10c-aa-intelligence-vs-cost-all-models.png]
The same chart with every lab plotted, if a wider establishing shot is wanted before zooming to the two models.
Source: same post
...
[IMAGE: refs/11-aa-intelligence-vs-output-tokens.png]
"Intelligence Index vs. Output Tokens per Intelligence Index Task", the same post's third chart. This is the
1.7x evidence: read the x positions of "Claude Fable 5.1 (max with fallback)" against "Claude Fable 5
(with fallback)" and "Claude Opus 5 (max)" on the log token axis.
Source: same post
...
[IMAGE: refs/08a-anthropic-quote-every.jpg]
Partner quote card 5 of 22 on the launch page, Every / Dan Shipper, CEO: "It's friendly Fable. Fable-level
intelligence, Opus-level price, Sonnet-speed. In our tests it was about twice as fast as Opus 5 and used half
as many tokens ...". The card is a green box with COMPANY and AUTHOR cells along the bottom.

[IMAGE: refs/08b-anthropic-quote-browserbase.jpg]
Card 16 of 22, Browserbase / Miguel Gonzalez, Technical Lead: 82% of tasks vs 74% for Opus 5 and 57% for
Fable 5, "while using fewer tokens than either".

[IMAGE: refs/08c-anthropic-quote-rogo.jpg]
Card 17 of 22, Rogo / Alex Wang, Applied AI: "matches Fable 5 on accuracy while using 20% fewer tokens".
Source for all three: https://www.anthropic.com/claude-fable-and-mythos-5-1
...
[IMAGE: refs/09-anthropic-tbs-accuracy-vs-cost.jpg]
The "Terminal-Bench-Science 0.1, Accuracy vs Cost" chart on the launch page (first tab of the switcher,
"Agentic scientific research"). Two curves, Fable 5.1 on top and Fable 5 below, each with five labelled dots:
low, med, high, xhigh, max. X axis is mean cost per task in USD on a log scale, Y axis is score.
Read Fable 5.1's "low" dot (about 26% at roughly $11) against Fable 5's "max" dot (about 24.7% at roughly $44).
Source: https://www.anthropic.com/claude-fable-and-mythos-5-1
...
## Link notes (read before recording)

1. **`docs.rs/crate/ccusage-rs/latest` does not exist.** docs.rs answers "The requested crate does not exist".
   Nothing on that URL matches the numbers in the narration (8K input, 20K output, 1.5M cache writes,
   16.6M cache reads, 92%). I substituted `refs/04a-ccusage-daily-token-breakdown.jpg` from
   https://docs.rs/crate/cc-usage/latest, which is a published Rust port of ccusage and shows a real daily row
   with the same four token columns (340 / 84,592 / 303,910 / 9,087,615, so about 96% cache reads).
   The story it tells is identical, the numbers are not the ones you say out loud. Either point to your own
   source for the 92% day, or read the numbers off this table instead.

2. **`artificialanalysis.ai/articles/opus-5` could not be screenshotted.** The domain is not enabled in the
   Claude in Chrome extension's site permissions. Allow it in the extension and this can be captured in a
   minute. Reading the page's text: it reports **Opus 5 (max) at 61 points and $2.03 per task**, while table 7
   in the script uses **63 and $2.34**, which is what the newer Artificial Analysis Fable 5.1 post states
   (that post re-ran Opus 5). Worth a sentence of care: cite the Fable 5.1 post for the Opus 5 numbers, not
   the older Opus 5 article, or the two sources will look like they contradict each other on screen.

3. **The Anthropic partner quotes are a carousel** (22 cards). The three you cite are card 5 (Every),
   card 16 (Browserbase) and card 17 (Rogo). The screenshots are of those exact cards, so if you film the page
   live you will need to click through to them.

4. Everything else in the script checks out against the live pages as captured on 2026-09-01, including the
   pricing rows, the benchmark values, the indexed cost chart's 100 to 75 and 100 to 55, and the GitHub MCP
   server's ~3,100 tokens per turn.
